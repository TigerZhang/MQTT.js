...
//  packet.messageId = parser.parse_num(buf);
    if (opts.protocolVersion == protocol.VERSION13) {
        packet.messageId = parser.parse_message_id64(buf);
        console.log("parse.subscribe 64bit message id", packet.messageId);
    } else {
        packet.messageId = parser.parse_message_id16(buf);
        console.log("parse.subscribe 16bit message id", packet.messageId);
    }
  if (packet.messageId === null) return new Error('Parse error - cannot parse message id');
...
parser.get = function(buf, packet, encoding, opts) {
    ...
    if (opts.protocolVersion == protocol.VERSION13) {
        packet.messageId = parser.parse_message_id64(buf);
        console.log("parse.get 64bit message id", packet.messageId);
    } else {
        packet.messageId = parser.parse_message_id16(buf);
        console.log("parse.get 16bit message id", packet.messageId);
    }
    if (packet.messageId === null) return new Error('Parse error - cannot parse message id');
...
parser.parse_string = function(buf) {
  ...
  if (length === null) {
      ...
      if (length + parser._pos > parser._len) {
          console.log('error length', length);
          length = parser._len - parser._pos;
      }
  }
