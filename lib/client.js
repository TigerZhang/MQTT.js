    ...
    switch (opts.qos) {
      ...
      case 1:
        if (opts.publishedTimeout !== undefined) {
            var myMid = packet.messageId;
            var that = this;
            var myTimer;
            var publishTimeout = opts.publishedTimeout;
            if (opts.publishedMsgId !== undefined) {
                packet.messageId = myMid = opts.publishedMsgId;
                delete opts.publishedMsgId;
            }
            delete opts.publishedTimeout;
            myTimer = publishTimer = setInterval(function() {
                that.publishTimeUp(myMid);
            }, publishTimeout);
            this._savetPublishTimer(myMid, myTimer);
        }
...
//            messageId: this._nextId()
        ...
        if (opts.data.publishedTimeout !== undefined) {
            var myMid = packet.messageId;
            var that = this;
            var myTimer;
            var publishTimeout = opts.data.publishedTimeout;
            if (opts.data.publishedMsgId !== undefined) {
                packet.messageId = myMid = opts.data.publishedMsgId;
                delete opts.data.publishedMsgId;
            }
            delete opts.data.publishedTimeout;
            myTimer = publishTimer = setInterval(function() {
                that.publishTimeUp(myMid);
            }, publishTimeout);
            this._savetPublishTimer(myMid, myTimer);
...
MqttClient.prototype.publishTimeUp = function(mid) {
    var myTimer = this._getPublishTimer(mid);
...
MqttClient.prototype._getPublishTimer =  function(message_id) {
    if (message_id !== undefined) {
        if (message_id in this.timerPublishCahce) {
            return this.timerPublishCahce[message_id];
        }
    }
    return undefined;
}

MqttClient.prototype._savetPublishTimer =  function(message_id, timer) {
    this.timerPublishCahce[message_id] = timer;
}

MqttClient.prototype._removePublishTimer =  function(message_id) {
    if (message_id !== undefined) {
        delete this.timerPublishCahce[message_id];
    }
}
