...
function json2buf(topics,payload,data) {
    ...
    for(var key in data) {
      ...
      //  console.log(key + ": " + data[key]);
        var strlen = Buffer.byteLength(data[key].toString());
        lenth += strlen;
    }
    ...
    for(var key in data) {
       // console.log(key + ": " + data[key]);
        ...
        inc_len = write_string(buff, pos, data[key].toString());
        pos += inc_len;
    }
    return buff;
};
...
function key2commad(key) {
    ...
    else if(key == 'time_reserve') return 3;
    else if(key == 'time_delay') return 4;
    else if(key == 'location') return 5;
    else if(key == 'qos') return 6;
    else {
