...
function Connection(protocolVersion) {
  if (!(this instanceof Connection)) {
    return new Connection(protocolVersion);
  }
...
Connection.prototype._parseHeader = function() {
  ...
  // For client, only parse getack
  if(this.packet.cmd == 'get'){
      this.options.ack = true;
  }
...
(function() {
  ...
  Object.keys(parse).forEach(function(key) {
    fun = fun +
    "   case '" + key + "': \n" +
    "     result = parse." + key + "(buf, this.packet, this._packetEncoding, this.options); \n" +
    "     break; \n ";
  });
